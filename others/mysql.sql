...
CREATE TABLE `heartbeat` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COMMENT='该表只有归档用处，仅供人工查阅。如果不需要服务器心跳记录归档，可将该表引擎类型设置为黑洞';
...
CREATE TABLE `order` (
  ...
  KEY `vpnid` (`vpnid`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COMMENT='订单表，保存生成的订单';
...
CREATE TABLE `service` (
  ...
  `radiusgroup` varchar(100) NOT NULL DEFAULT '' COMMENT 'RADIUS 中对应的用户组名字',
...
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COMMENT='服务（商品）列表及说明';
